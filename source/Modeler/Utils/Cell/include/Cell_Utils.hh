...
Point_3 getCentroid(TIteratorOnConstCell pItStart, TIteratorOnConstCell pItEnd) {
	std::vector<Point_3> cellsPositions;
	for(auto it = pItStart; it != pItEnd; ++it)
...
BoundingBox<Point_3> getBoundingBox(IIt begin, IIt end) {
	...
	for(auto it = begin; it != end; ++it) {
		auto const* lCell = dynamic_cast<const SpheroidalCell*>(*it);
		if(lCell)
			maxCellRoundCellRadius = std::max(lCell->getRadius(), maxCellRoundCellRadius);
	...
	return {
		...
		Point_3(bb.xmax()+2*maxCellRoundCellRadius, bb.ymax()+2*maxCellRoundCellRadius, bb.zmax()+2*maxCellRoundCellRadius)
