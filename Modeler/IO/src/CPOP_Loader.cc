...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
map<unsigned long int, CellProperties* > CPOP_Loader::getCellPropertiesMap(QXmlStreamReader* xmlReader)
{
    assert(xmlReader);
    map<unsigned long int, CellProperties* > lCellPropertiesMap;
        ...
        if(xmlReader->name() == all_cell_properties_flag)
        {
            while( !xmlReader->atEnd() &&  !xmlReader->hasError() && (xmlReader->name() != CLOSING_XML_NODE(all_cell_properties_flag)))
            {
                ...
                if(xmlReader->name() == cell_property_flag)
                {
                    ...
                    lCellPropertiesMap.insert( make_pair( lCellPropID, lCellProperty) );
                }
            }
        }
    }
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
map<LifeCycles::LifeCycle, CellProperties::t_CellVarAtt_d > CPOP_Loader::getCellProp_d_Value(QXmlStreamReader* xmlReader, QString closingNodeName)
{
    assert(xmlReader);
    assert(xmlReader->name() == closingNodeName);
    xmlReader->readNext();

    map<LifeCycles::LifeCycle, CellProperties::t_CellVarAtt_d > result;
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
map<LifeCycles::LifeCycle, G4Material* > CPOP_Loader::getCellProp_Mat_Value(QXmlStreamReader* xmlReader, QString closingNodeName)
{
    assert(xmlReader);
    assert(xmlReader->name() == closingNodeName);
    xmlReader->readNext();

    map<LifeCycles::LifeCycle, G4Material* > result;
...
///
///////////////////////////////////////////////////////////////////////
vector<nucleiInfo> getNucleiInfo(QXmlStreamReader* xmlReader)
{
    assert(xmlReader->name() == nuclei_flag);
    xmlReader->readNext();
    vector<nucleiInfo> result;
    while(xmlReader->name() != nuclei_flag)
    {
        ...
        if(xmlReader->name().toString() == nucleus_flag)
        {
            ...
            result.push_back(nucleiInfo(lNucleusRadius, lPositionType, lNucleusType));
...
DiscoidalCell* CPOP_Loader::createDiscoidalCell(QXmlStreamReader* xmlReader, bool forceID)
{
    ...
    if(cellPropertiesMap.find(lCellPropertyID) == cellPropertiesMap.end() )
    {
    ...
    {
        cellProperties = cellPropertiesMap.find(lCellPropertyID)->second;
    }
    ...
    if(nucleiInfo.size() == 1 && (nucleiInfo.front().nucleusType == 0)) // todo : set round nucleus wil be better
    {
        ...
        SimpleDiscoidalCell* cell = new SimpleDiscoidalCell(cellProperties, position, cellRadius, nucleusRadius, nucleusPosType, mass);
...
SpheroidalCell* CPOP_Loader::createSpheroidalCell(QXmlStreamReader* xmlReader, bool forceID)
{
    ...
    if(nucleiInfo.size() == 1 && (nucleiInfo.front().nucleusType == 0)) // todo : set round nucleus wil be better
    {
        ...
        /// \warning : we give the nucleusPosType because can be random on cellProperties and otherwise could be changed from a save file to a load file
        SimpleSpheroidalCell* cell = new SimpleSpheroidalCell(cellProperties, position, cellRadius, nucleusRadius, nucleusPosType, mass);
