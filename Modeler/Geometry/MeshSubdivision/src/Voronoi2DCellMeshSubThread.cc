...
void Voronoi2DCellMeshSubThread::clean()
{
	...
	// delete intersection
	std::vector<Line_2*>::iterator itLine;
	for(itLine = intersections.begin(); itLine != intersections.end(); ++itLine)
	{
		delete (*itLine);
	}
	intersections.clear();
...
{
	assert(cell);
	assert(MIN_DISC_POINT> 1);
	/// TODO : remove RefinementThread::numberOfUnitaryEntityPerCell
	std::vector<Point_2> polyInitPts = Utils::Geometry::Circle::getNPointOnCircle(cell->getOrigin(), cell->getRadius(), RefinementThread::numberOfUnitaryEntityPerCell);	// TODO : MIN_PT
...
void Voronoi2DCellMeshSubThread::adjustPointFromIntersections(Point_2& ptToCheck, std::vector<const Line_2*>* lIntersections, DiscoidalCell* cell )
{
	for(unsigned int iLine = 0; iLine < lIntersections->size(); iLine ++)
	{
		...
		if(lIntersections->at(iLine)->oriented_side(ptToCheck) == CGAL::ON_NEGATIVE_SIDE)
		{
			...
			return adjustPointFromIntersections(ptToCheck, lIntersections, cell);
		}
	}
}
...
void Voronoi2DCellMeshSubThread::adjustPointFromIntersections(Point_2& ptToCheck, DiscoidalCell* cell)
{
	...
	for(std::vector<Line_2*>::iterator itLine = intersections.begin(); itLine != intersections.end(); ++itLine)
	{
		lIntersections.push_back(const_cast<const Line_2*>(*itLine));
	}
