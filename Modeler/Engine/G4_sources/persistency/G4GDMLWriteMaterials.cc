...
///
   ...
   {
      const G4int* nbAtomPerElement = materialPtr->GetAtomsVector();
      bool mustBeImportedByFraction = false;
      for (size_t i=0;i<NumberOfElements;i++)
      {
         if(nbAtomPerElement[i] < 1)
         {
            mustBeImportedByFraction = true;
            break;
         }
      }

      if(mustBeImportedByFraction)
      {
      ...
      }else
      {
         ...
         for (size_t i=0;i<NumberOfElements;i++)
         {
            G4String fractionref = GenerateName(materialPtr->GetElement(i)->GetName(),
                                                materialPtr->GetElement(i));
            xercesc::DOMElement* fractionElement = NewElement("composite");
