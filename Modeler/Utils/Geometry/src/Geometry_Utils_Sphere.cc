...
namespace Utils
{
	namespace Geometry
	{
		namespace Sphere
		{
			...
			std::vector<Point_3> getNPointOnSphere( Point_3 sphere_center, double sphere_radius, unsigned int sphere_degree)
			{
				...
				for(unsigned int lat = 0; lat < sphere_degree; lat++)
				{
					for(unsigned int hor = 0; hor < sphere_degree; hor++)
					{
						...
						result.push_back( Point_3( 	sphere_center.x() +	sphere_radius*std::cos( gamma ) * std::sin(theta),
			...
			////////////////////////////////////////////////////////////////////////////////////////        
	        int lineSphereIntersection(Segment_3 seg, Point_3 sphereCenter, double sphereRadius,
	        		Point_3& intersection1, Point_3& intersection2)
	        {
			...
			/// \param intersection2 The second potential intersection point
			////////////////////////////////////////////////////////////////////////////////////////
	        int segmentSphereIntersection(Segment_3 seg, Point_3 sphereCenter, double sphereRadius,
	        		Point_3& intersection1, Point_3& intersection2)
	        {
	        	assert( seg.source() != seg.target());
			...
			Point_3 getSpotOnSphere(double radius, Point_3 center, double internalRadius)
			{
				...
				return Point_3(p.x()+center.x(), p.y()+center.y(), p.z()+center.z());
			}

		} // end namespace Sphere
