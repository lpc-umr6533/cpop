	...
	std::map<LifeCycles::LifeCycle, double > pLifeCycles)
{
	...
	std::map<LifeCycles::LifeCycle, double>::const_iterator itLSR;
	int nbCellGenerated = 0;

	std::map<LifeCycles::LifeCycle, double>::const_iterator lastState = pLifeCycles.end();
	lastState --;
	for(itLSR = pLifeCycles.begin(); itLSR != pLifeCycles.end(); ++itLSR)
	{
		...
		{
			Cell<Kernel, Point, Vector>* newCell = CellFactory::getInstance()->produce<Kernel, Point, Vector>( pCellProperties, itLSR->first);
	...
	std::map<LifeCycles::LifeCycle, double > pLifeCycleRates)
{
	...
	std::map<LifeCycles::LifeCycle, double >::const_iterator itLC = pLifeCycleRates.begin();
	...
	for(itNucleus = nuclei.begin(); itNucleus != nuclei.end(); ++itNucleus)
	{
		...
		RoundNucleus<Kernel, Point, Vector>* nucleus = static_cast<RoundNucleus<Kernel, Point, Vector>*> (*itNucleus);
		if(nucleus)
		{
			// set nucleus radius
	...
	const RoundCellProperties* roundProp = dynamic_cast<const RoundCellProperties*> (pCell->getCellProperties());
	if(roundProp)
	{
...
#endif // RANDOM_CELL_DISTRIBUTION_HH