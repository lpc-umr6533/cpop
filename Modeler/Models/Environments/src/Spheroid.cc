...
t_Mesh_3* Spheroid<SimpleSpheroidalCell>::getMesh(int* error, unsigned int pNbMaxFacetPerPoly, double pDdeltaReffinement)
{
	...
	return MeshFactory::getInstance()->create_3DMesh(error, spheroidSubEnvironment, MeshTypes::Round_Cell_Tesselation, pNbMaxFacetPerPoly, pDdeltaReffinement);
}
...
BoundingBox<Point_3> Spheroid<SimpleSpheroidalCell>::getBoundingBox() const
{
	...
	return BoundingBox<Point_3>( 
		...
		Point_3(bb.xmax()+maxRadius, bb.ymax()+maxRadius, bb.zmax()+maxRadius) );	// for now weight correspond to the cell radius.
}
