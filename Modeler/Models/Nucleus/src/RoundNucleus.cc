...
G4PVPlacement* RoundNucleus<double, Point_3, Vector_3>::convertToG4Entity(QString name, G4LogicalVolume* motherVolume, G4Material* pNucleusMat, bool checkOverLaps) const
{
	...
	G4PVPlacement* physVol = new G4PVPlacement(	G4Translate3D(origin.x()*convertToG4, origin.y()*convertToG4, origin.z()*convertToG4),			// no rotation
						                        logicVol,     								// its logical volume
												physVolName.toStdString(), 					// its name
						                        (motherVolume == NULL ) ? 0 : motherVolume,	// its mother  volume
...
G4LogicalVolume* RoundNucleus<double, Point_3, Vector_3>::convertToG4LogicalVolume(QString name, G4Material* pNucleusMat) const
{
	...
	G4Orb* nucleusSolid = new G4Orb(name.toStdString(), getRadius()*convertToG4);

	assert(nucleusSolid);

	QString logicalVolName = "LV_" + name;

	// std::cout << '\n' << " logicalVolName " << printf(logicalVolName.toStdString().c_str()) <<'\n';

	return new G4LogicalVolume(nucleusSolid, pNucleusMat, logicalVolName.toStdString(), 0, 0, 0);
}
...
{
	return Utils::Geometry::Sphere::getSpotOnSphere(radius, origin);
}
...
{
	return Utils::Geometry::Circle::getSpotOnDisc(radius, origin);
}
...
double RoundNucleus<double, Point_3, Vector_3>::getMeshVolume(MeshOutFormats::outputFormat meshType) const
{
	switch(meshType)
	{
		...
		{
			std::vector<Point_3> lCellNucleusPoints = getShapePoints();
